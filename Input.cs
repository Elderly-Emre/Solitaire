				...
				if (Physics.Raycast(GameManager.GetMainCamera().transform.position, GameManager.GetMainCamera().transform.TransformDirection(Vector3.forward), out hit, 2f, layerMask))
				{
					...
					if (InputManager.GetMouseButtonDown(InputManager.m_CurrentContext, 0)) // Left mouse button            
					{
						if (hitObjectName == "Playmat")
						{
                            ...
                            Vars.currentGame.cardAudioShot.PlayOneshot(SolitaireMain.cardAudioManager.GetClip("cardshove4"));
                        }
						...
						if (!Vars.cardInHand)
						{
							if (hitObjectName == "controlNewGame")
							{
                                ...
                                Vars.currentGame.cardAudioShot.PlayOneshot(SolitaireMain.cardAudioManager.GetClip("cardfanmp3"));

                                Vars.currentGame.stackInitialized = true;
							}
							else if (hitObjectName == "controlLeaveGame")
							{                            
                                Vars.currentGame.cardAudioShot.PlayOneshot(SolitaireMain.cardAudioManager.GetClip("chipshandle1"));
                                Actions.ExitGameView(Vars.currentGame);
							}
							else if (hitObjectName == "controlStowCards")
							{
                                Vars.currentGame.cardAudioShot.PlayOneshot(SolitaireMain.cardAudioManager.GetClip("cardopenpackage2"));
                                Actions.ExitGameView(Vars.currentGame);
								
								GameManager.GetPlayerManagerComponent().ProcessPickupItemInteraction(Vars.currentGameObject.GetComponent<GearItem>(), false, false, false);
							}
